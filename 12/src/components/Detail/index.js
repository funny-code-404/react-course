...
class Detail extends React.Component{
	render() {
		...
		const arrs = [{ army_type }, { team_bonus }, { description }, { expansion }, { age }, { build_time }, { reload_time }, { attack_delay }, { movement_rate }, { line_of_sight }, { hit_points }, { range }, { attack }, { armor }, { search_radius }, { accuracy }];
				...
				<div className='ditail-block'>
					{
						arrs.map(x => {
							for(const key in x){
								return(
									x[key] &&
									<p key={key}>{key}: <span>{x[key]}</span></p>
								)
							}
						})
					}
					...
					{
						civilization_bonus && <CivilizationBonus data={civilization_bonus} />
					}
					{
						unique_unit && Boolean(unique_unit.length) && <DetailUniqueUnit data={unique_unit} />
					}
					{
						unique_tech && Boolean(unique_tech.length) && <DetailUniqueTech data={unique_tech} /> 
					}
					{
						created_in && Boolean(created_in.length) && <DetailCreatedIn data={created_in}  />
					}
				</div>
			</div>
		)
